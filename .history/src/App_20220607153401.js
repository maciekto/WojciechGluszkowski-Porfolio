...
function App() {
  ...
  let [HeaderwDisplay, setHeaderwDisplay] = useState()
  ...
  const Main = () => {
    ...
    setTimeout(() => {
      setHeaderwDisplay(HeaderwDisplay = 'App-Header-None')
    }, 800);
  ...
  return (
    <div className="App">
      <header className={`App-Header ${HeaderClickedClass} ${HeaderwDisplay}`}>
      ...
      <div className={`App-Popup ${Popup}`} onClick={ClosePopup}>
        <img src={Image} className='App-Popup-Image'/>
      </div>
    </div>
  );
}
